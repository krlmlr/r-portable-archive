...
```{r echo = FALSE, results = "asis"}
pkgs <- as.data.frame(installed.packages(.Library), stringsAsFactors = FALSE)[c("Package", "Version", "Priority")]
...
plyr::d_ply(
  ...
  function (sub_pkgs) {
    ...
    s <- plyr::dlply(
      ...
      "Package",
      function(pkg) {
        sprintf("[`%s`](cran.r-project.org/package=%s) (%s)", pkg$Package, pkg$Package, pkg$Version)
      }
    )
